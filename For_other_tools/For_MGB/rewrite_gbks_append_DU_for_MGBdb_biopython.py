# the script is to overcome certain bug in MGB - multi gene blast program
# http://multigeneblast.sourceforge.net/
# I believe there is better version of MGB in current antiSMASH suit, but the
# multigeneblast program in there is not released.
...
def rewrite_gbk_append(infolder, outfolder):
    genomes = [file for file in os.listdir(infolder) if os.path.splitext(file)[
        1] in ['.gb', '.gbk', '.genbank']]
    geneSeen = {}
    proteinSeen = {}
    totalAll = 0
