# Generate a table corresponding to the function of each SCO proteins.
# Result will be put into an excel file.
...
with open(output, 'w') as file_handle:
    ...
    for idx, feat in enumerate(M145.features):
        # Unlike "CDS", "gene" type should contain the full list of SCO numbers, including those pseudo genes.
        if feat.type == 'gene':
            # location fixed for identify 'CDS' or 'misc_feature' is for the same gene.
        ...
        elif feat.type == 'CDS' and feat.location.start == location.start and feat.location.end == location.end and feat.location.strand == location.strand:
            ...
            if 'db_xref' in feat.qualifiers:
                for db_xref in feat.qualifiers['db_xref']:
                    if db_xref.startswith('GI:'):
                        ...
                        GeneID = db_xref.split(':')[1]
            # Grab the last item in note, which is normally a brief description of predicted function
            product = feat.qualifiers['note'][0].split(';')[-1]
            translation = ''
            print(f"{locus_tag}\t{protein_id}\t{GI}\t{GeneID}\t{product}\t{translation}", file=file_handle)
            clear()
