# This script is to build relationship between uniprotID with SCO number
# with an ambiguous blastp method
...
def writeProtUniprot(ids, output):
    '''Fetch protein information (raw) and write to fasta file.
    Run in parallel.'''
    seqList = []
    notFound = []
    maxConnections = 5
...
def findSco(queryFile, logFile):
    '''findSco(queryFile, logFile)
    queryFile should contain several *protein* sequences as fasta format
    will do blastp and then parse result
    return dictFound[uniprotId] = scoId'''
    dictFound = {}
            ...
            if scoId == '':
                for hsp in alignment.hsps:
                    if 0.98 * blastRec.query_length < hsp.identities < 1.02 * blastRec.query_length:
                        # set threshold for identify, should be around 100%
                        pass
                    else:
                    ...
                    # for both genome and plasmid IDs
                    if hsp.align_length / blastRec.query_length != 1:  # Only record when it is not identical
                        print('*' * 100, file=log)
